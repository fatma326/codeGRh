...
export class CnssComponent implements OnInit {
  ...
  private url  = 'http://localhost:8080/CnssList';
  ...
  constructor(private httpClient: HttpClient, private dialog: MatDialog) {
    ...
    this.columnDefs = [
      {headerName: 'ID', field: 'idCnss', sortable: true, filter: true},
      {headerName: 'pourcentageCnss', field: 'pourcentageCnss', sortable: true, filter: true },
  ...
  post() {
    this.httpClient.post(this.url, this.item)
      .subscribe(data => {
        //this.employer = data;
        console.log("cnss a ete ajouter !!")
        this.submitted = true;
  ...
  put() {
    this.httpClient.put(this.url + '/'+this.item.idCnss, this.item)
      .subscribe(data => {
        //this.employer = data;
        console.log("cnss a ete mis a jour !!")
        this.submitted = true;
        this.refreshDataGrid();
      }, err => {
        console.log(err);
  ...
  delete() {
    this.httpClient.delete(this.url + '/'+this.item.idCnss)
      .subscribe(data => {
        //this.employer = data;
        console.log("cnss a ete supprimer !!")
        this.submitted = true;
        this.mode = 'supprimer'
        this.refreshDataGrid();
      }, err => {
