...
import {EditPrimeVComponent} from './edit-prime-v/edit-prime-v.component';
...
export class PrimevariableComponent implements OnInit {
  ...
  ajouter(){
    ...
    const dialogRef = this.dialog.open(EditPrimeVComponent, {
      data: {
        item: this.item,
        btn: 'Ajouter'
      }
  ...
  delete() {
    this.httpClient.delete(this.url + '/'+this.item.idprimeV)
      .subscribe(data => {
        //this.employer = data;
        console.log("prime variable a ete supprimer !!")
        this.submitted = true;
        this.mode = 'supprimer'
        this.refreshDataGrid();
      }, err => {
